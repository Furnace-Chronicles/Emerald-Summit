...
/obj/structure/autosmither
	...
	anchored = TRUE
	density = TRUE
	layer = ABOVE_MOB_LAYER + 0.01 // draw over adjacent cogs/shafts (which sit at ABOVE_MOB_LAYER), like the infernal engine
...
/obj/structure/autosmither/ui_data(mob/user)
	...
	data["rpm"] = rotations_per_minute
	// Skilled engineers (level 4+) skip the lever/button/dial sequence and just flip it on/off.
	data["can_quick_toggle"] = user?.get_skill_level(/datum/skill/craft/engineering) >= 4
	return data
...
/obj/structure/autosmither/attackby(obj/item/I, mob/living/user, params)
	. = ..()
	if(.)
		return
	if(!working)
...
// Level 4+ engineers bypass the lever/button/dial sequence — a plain on/off switch instead.
/obj/structure/autosmither/proc/quick_toggle(mob/living/user)
...
/obj/structure/autosmither/update_animation_effect()
	...
	if(!bloodied)
		if(!show_working_anim())
			...
			return
		var/frame_stage = 32 / max(1, rotations_per_minute) // full 5-frame cycle: 1s at 16 RPM, 0.5s at 32 RPM
		if(rotation_direction == WEST)
			// first animate step must differ from the resting state or BYOND discards the animation
		...
		if(rotation_direction == WEST)
			// first animate step must differ from the resting state or BYOND discards the animation
			animate(src, icon_state = "b2", time = frame_stage, loop=-1)
