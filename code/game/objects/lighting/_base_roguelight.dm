/obj/machinery/light/rogue
	...
	var/datum/status_effect/buff/stamina_status_effect = null
...
/obj/machinery/light/rogue/process()
	. = ..()
	if(!on || !healing_range || !stamina_status_effect)
		return .
...
/obj/machinery/light/rogue/attackby(obj/item/W, mob/living/user, params)
	...
	if(W.firefuel && !no_refuel)
		if(W.smeltresult) // For things with actual smelt results - functionally no differences
			if(alert(usr, "Fuel [src] with [W]?", "EMERALD SUMMIT", "Fuel", "Smelt") != "Fuel")
				return TRUE
		if(alert(usr, "Fuel [src] with [W]?", "EMERALD SUMMIT", "Yes", "No") != "Yes")
			return TRUE
		if(!W)
			return
