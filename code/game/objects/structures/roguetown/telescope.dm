...
/obj/structure/telescope/attack_hand(mob/user)
	...
	var/random_message = pick("You can see noc rotating.", "Looking at Sarael blinds you!", "The stars smile at you.", "Nepolx is red today.", "Blessed yellow strife.", "You see a star!")
	to_chat(H, span_notice("[random_message]"))

	if(random_message == "Looking at Sarael blinds you!")
...
/obj/structure/globe/attack_hand(mob/user)
	...
	var/mob/living/carbon/human/H = user
	var/random_message = pick("you spin the globe!", "You land on Scarlet Reach!", "You land on Shalvistine!", "You land on port Ice cube!.", "You land on port Thornvale!", "You land on grenzelhoft!")
	to_chat(H, span_notice("[random_message]"))
