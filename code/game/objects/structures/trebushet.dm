...
/obj/structure/trebuchet/update_icon()
	icon_state = loaded ? "trebuchet" : "trebushetempty"
...
/obj/structure/trebuchet/proc/aim_and_fire_ui(mob/living/user)
	if(world.time < last_fire + firing_cooldown)
		...
		return TRUE
	if(!loaded || !loaded_ammo_path)
		to_chat(user, span_warning("Сначала нужно загрузить снаряд."))
		return TRUE
	var/tx = input(user, "Координата X цели:", "Trebuchet", src.x) as null|num
	if(isnull(tx)) return TRUE
	var/ty = input(user, "Координата Y цели:", "Trebuchet", src.y) as null|num
	if(isnull(ty)) return TRUE
	var/default_z = z
	if(default_z < 1 || default_z > world.maxz)
		default_z = 1
	var/tz = input(user, "Уровень Z цели (1..[world.maxz]):", "Trebuchet", default_z) as null|num
	...
	if(!columns_clear_between(src.x, src.y, z, tz))
		to_chat(user, span_warning("Над требушетом есть препятствия. Очистите пространство между уровнями [min(z,tz)+1]..[max(z,tz)]."))
		return TRUE
	if(!columns_clear_between(tx, ty, tz, z))
		to_chat(user, span_warning("Над целью есть препятствия. Очистите пространство между уровнями [min(z,tz)+1]..[max(z,tz)]."))
		return TRUE
	fire(user, target)
	return TRUE

/obj/structure/trebuchet/proc/fire(mob/living/user, turf/target)
	if(!loaded || !loaded_ammo_path || !target)
		return
	...
	M.update_maptext()
	if(fire_sound)
		playsound(src, fire_sound, 80, FALSE)
	visible_message(span_warning("[src] запускает снаряд!"))
	addtimer(CALLBACK(src, PROC_REF(resolve_impact), target, loaded_ammo_path), impact_delay)
	loaded = FALSE
	loaded_ammo_path = null
	update_icon()

/obj/structure/trebuchet/proc/resolve_impact(turf/T, path_type)
	if(!T)
		return
	switch(path_type)
