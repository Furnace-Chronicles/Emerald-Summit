...
/obj/item/natural/stone/attackby(obj/item/W, mob/living/user, params)
	...
	if( user.used_intent.type == /datum/intent/chisel )
		...
		if(do_after(user, work_time))
			...
			new /obj/item/natural/stoneblock(work_turf)
			if(HAS_TRAIT(user, TRAIT_MASTER_MASON)) //double the amount for any in a stone worker role
				new /obj/item/natural/stoneblock(work_turf)
			new /obj/effect/decal/cleanable/debris/stony(work_turf)
			playsound(work_turf, pick('sound/combat/hits/onrock/onrock (1).ogg', 'sound/combat/hits/onrock/onrock (2).ogg', 'sound/combat/hits/onrock/onrock (3).ogg', 'sound/combat/hits/onrock/onrock (4).ogg'), 100)
			qdel(src)
			user.mind.add_sleep_experience(/datum/skill/craft/masonry, (user.STAINT*0.2))
			chisel_chain_next(user, W, work_turf) // Emerald: auto-continue with next chiselable on this turf.
		return
