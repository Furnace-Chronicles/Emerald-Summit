...
/datum/objective/nobility/proc/on_nobility_granted()
	...
	to_chat(owner.current, span_greentext("You have earned nobility and completed Sarael's objective!"))
	owner.current.adjust_triumphs(2)
	completed = TRUE
	adjust_storyteller_influence("Sarael", 15)
	escalate_objective()
	UnregisterSignal(owner.current, SIGNAL_ADDTRAIT(TRAIT_NOBLE))

/datum/objective/nobility/update_explanation_text()
	explanation_text = "Become part of the nobility by any means to gain Sarael's approval!"
