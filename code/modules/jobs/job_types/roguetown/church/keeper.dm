/datum/job/roguetown/keeper
	...
	total_positions = 2
	spawn_positions = 2
	allowed_races = RACES_ALL_KINDS //ES: substitutes AP's ACCEPTED_RACES (not defined here).
	...
	//ES: AP gated medicine/sewing/survival/homestead via dedicated EXPERT cap-traits. ES only trait-gates
	//alchemy (via TRAIT_TALENTED_ALCHEMIST) and firearms; the other crafts advance freely, so only the
	//alchemy gate is needed here.
	job_traits = list(TRAIT_TALENTED_ALCHEMIST, TRAIT_NOSTINK,
...
/datum/advclass/keeper
	...
	//ES: AP capped post-statpack STR at 6 via advclass adv_stat_ceiling, which ES's advclass does not
	//support. The -5 STR penalty above still applies; the anti-statpack cap is omitted.
