...
/mob/living/carbon/human/proc/apply_gnoll_preferences(initial_setup = TRUE)
	...
	if(initial_setup)
		// Gnolls should be a blank slate at spawn; strip inherited charflaw state from base character prefs.
...
/mob/living/carbon/human/proc/clear_non_gnoll_bodyparts()
	return // ES has no taur species; stripped from upstream port. Kept as no-op stub for call sites.
