/datum/advclass/wretch/maestro
	...
	outfit              = /datum/outfit/job/wretch/maestro
	...
	extra_context       = "All paths gain Allegro: every 5th landed rhythm restores energy.<br>\
						  ...
						  Cacophonist gains a bonus rhythm pick (3 total) and song slot (5 total), with Expert Swords and Whips & Flails (Whip & Arming Sword).<br>\
...
/datum/outfit/job/wretch/maestro/pre_equip(mob/living/carbon/human/H)
	..()
	...
	I.allegro_enabled = TRUE

	if(H.mind)
		H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/vicious_mockery)

		var/instruments = list("Harp","Lute","Accordion","Guitar","Hurdy-Gurdy","Viola","Vocal Talisman","Flute","Drums","Psyaltery")
		...
		switch(path_choice)
			...
			if("Cacophonist")
				H.inspiration.bonus_rhythm_picks = 1
				H.inspiration.maxsongs++
			...
			if("Chanter")
				...
				D.grant_miracles(H, cleric_tier = CLERIC_T1, passive_gain = CLERIC_REGEN_DEVOTEE, devotion_limit = CLERIC_REQ_1)
			if("Cipher")
				...
				switch(poke_choice)
					if("Spitfire")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/spitfire_magi2)
					if("Frost Bolt")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/frost_bolt_magi2)
					if("Arc Bolt")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/arc_bolt_magi2)
					if("Greater Arcyne Bolt")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/greater_arcyne_bolt_magi2)
					if("Arcyne Lance")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/arcyne_lance_magi2)
					if("Gravel Blast")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/gravel_blast_magi2)
					if("Soulshot")
						H.mind.AddSpell(new /datum/action/cooldown/spell/projectile/soulshot_magi2)
