...
/obj/effect/proc_holder/spell/invoked/engineertuneup/cast(list/targets, mob/living/user)
	...
	//winding up only works on constructs (ES adaptation: construct mob typepath instead of RW's species datum)
	if(istype(targets[1], /mob/living/carbon/human/species/construct))
		...
		if(human_target in range(1, user))
			...
			if(human_target.stat != DEAD)
				...
				if(do_after(user, 10 SECONDS, target = human_target))
	...
	if(isstructure(targets[1]))
		if(is_type_in_list(targets[1], repairablelist))
			...
			if(do_after(user, 10 SECONDS, target = structurerepair))
	...
	if(isitem(targets[1]))
		...
		if(ispath(contraptionrepair.type, /obj/item/contraption))
			...
			if(do_after(user, 3 SECONDS, target = contraptionrepair))
