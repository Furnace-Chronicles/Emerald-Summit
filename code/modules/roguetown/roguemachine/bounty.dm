...
/obj/structure/chair/freedomchair
	name = "LIBERTAS"
	desc = "A chair-shaped machine normally used to place cursed collars onto a prisoner's neck. This one's been tampered with, and now does the opposite - re-purposed to remove those wretched magical collars."
...
/obj/structure/chair/freedomchair/crafted/attack_right(mob/living/carbon/human/A)
	...
	if(old_mask)
		if(istype(old_mask, /obj/item/clothing/neck/rogue/gorget/prisoner))
...
/obj/structure/chair/arrestchair/attack_right(mob/living/carbon/human/A)
	...
	if(correct_head)
		...
		if(old_mask)
			M.dropItemToGround(old_mask, TRUE)
		var/obj/item/clothing/neck/rogue/gorget/prisoner/prisonmask = new(get_turf(M))
...
/obj/structure/chair/arrestchair/proc/giveup(mob/living/carbon/human/M)
	if(alert(M, "Do you submit to the Collar, or do you die? You have 10 seconds to decide.", "CHOICE OF LYFE", "Submit", "Perish") == "Perish")
