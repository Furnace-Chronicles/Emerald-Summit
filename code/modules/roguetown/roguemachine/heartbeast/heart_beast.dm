...
/obj/structure/roguemachine/chimeric_heart_beast/ui_data(mob/user)
	...
	// Already-researched (unlocked) nodes, so the player can review what the beast knows.
	var/list/researched_data = list()
	for(var/node_id in SSchimeric_tech.all_tech_nodes)
		var/datum/chimeric_tech_node/N = SSchimeric_tech.all_tech_nodes[node_id]
		if(!N.unlocked)
			continue
		UNTYPED_LIST_ADD(researched_data, list(
			"name" = N.name,
			"desc" = N.description,
			"path" = N.string_id,
			"required_tier" = N.required_tier,
