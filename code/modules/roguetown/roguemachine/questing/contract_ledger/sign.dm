...
/obj/structure/roguemachine/contractledger/proc/turn_in_scroll(mob/user, obj/item/quest_writ/scroll)
	...
	// ES deviation: integer ledger, so the gross is credited straight onto the bearer's balance.
	SStreasury.bank_accounts[user] += gross_reward
	...
	if(quest_levy_exempt)
		...
		if(isliving(user) && SStreasury.is_tax_exempt(user, TAX_CATEGORY_CONTRACT_LEVY))
			SStreasury.record_tax_exemption(TAX_CATEGORY_CONTRACT_LEVY, FLOOR(base_reward * base_rate, 1))
		else
			var/rate = base_rate
			if(isliving(user))
				rate = min(rate, SStreasury.get_rate_cap(user, TAX_CATEGORY_CONTRACT_LEVY))
			if(rate < base_rate)
	...
	var/guild_fee_paid = pay_innkeeper_referral_fees(user, completed_quest, gross_reward)
