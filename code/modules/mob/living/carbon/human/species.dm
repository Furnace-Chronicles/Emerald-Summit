	...
	switch(damagetype)
		if(BRUTE)
			...
			if(!HAS_TRAIT(H, TRAIT_NOPAIN))
				if(damage_amount > 5)
					...
					if(prob(damage_amount * 3))
						if(damage_amount > ((H.STACON*10) / 3))
							...
							H.emote("pain")
				if(damage_amount > ((H.STACON*12.5) / 3) && !HAS_TRAIT(src, TRAIT_NOPAIN) && !HAS_TRAIT(src, TRAIT_NOPAINSTUN)) //We want this effect only on heavy hits.
				...
				if(damage_amount > 10 && !HAS_TRAIT(src, TRAIT_NOPAIN) && !HAS_TRAIT(src, TRAIT_NOPAINSTUN))
