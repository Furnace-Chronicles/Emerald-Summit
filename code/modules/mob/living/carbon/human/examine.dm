...
//		. += "[m3] [back.get_examine_string(user)] on [m2] back."
	...
	// Slave gorget phrases
	if(istype(wear_neck, /obj/item/clothing/neck/roguetown/gorget/slave_gorget))
		var/obj/item/clothing/neck/roguetown/gorget/slave_gorget/G = wear_neck
	...
	if(wear_ring && !(SLOT_RING in obscured))
		...
		if(istype(wear_ring, /obj/item/clothing/ring/slave))
			...
			if(lowertext(user.job) == "grand duke" || lowertext(user.job) == "grand duchess")
				...
				. += "<br><b>Death:</b> \"[R.death_phrase]\""
				. += "<br><b>Submission:</b> \"[R.submission_phrase]\""
				. += "<br><b>Freedom:</b> \"[R.freedom_phrase]\""
