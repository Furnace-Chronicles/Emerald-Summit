...
/datum/liquid_group/proc/process_member(turf/member)
	if(istype(member, /turf/open/lava))
...
/datum/liquid_group/proc/try_bulk_split(list/input_turfs)
	...
	for(var/turf/listed_input in input_turfs)
		for(var/turf/cardinal in listed_input.atmos_adjacent_turfs)
...
/datum/liquid_group/proc/spread_liquid(turf/new_turf, turf/source_turf)
	...
	if(QDELETED(new_turf.liquids) && !istransparentturf(new_turf) && !istype(new_turf, /turf/open/water) && source_turf.turf_height == new_turf.turf_height) // no space turfs, or oceans turfs, also don't attempt to spread onto a turf that already has liquids wastes processing time
