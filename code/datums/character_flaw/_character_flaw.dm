...
// Ported from AP #6849. ES deviations: bank_accounts are integer balances keyed by mob (not
...
// leaves the realm instead of minting back into the Crown's Purse). Bounty goes through ES's
	...
	if(bankamt > minimum)
		...
		if(!bounty_added)
			if(ishuman(deadbeat))
				...
				add_bounty_noface(H.real_name, H.dna.species, H.gender, height, body, voice, rand(100, 200), FALSE, "Failure to pay outstanding debts.", "The Justiciary of the Summit")
			bounty_added = TRUE
