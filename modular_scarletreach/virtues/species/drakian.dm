...
/datum/virtue/racial/drakian/horde_keeper/apply_to_human(mob/living/carbon/human/recipient)
	// apply horde keeper status effect
	recipient.apply_status_effect(/datum/status_effect/horde_keeper)
...
/datum/virtue/racial/drakian/ember_blooded
	...
	custom_text = "Only available to drakians."
	required_virtues = list(/datum/virtue/racial/drakian/horde_keeper)
	races = list(
		/datum/species/dracon,
	)
...
/datum/status_effect/horde_keeper
	id = "horde_keeper"
	status_type = STATUS_EFFECT_UNIQUE
	alert_type = null
	tick_interval = 5 SECONDS
	// You'll need growing amounts of mammon to satisfy the horde.
	/// Amount of mammons required to satisfy the horde keeper
...
/datum/status_effect/horde_keeper/on_creation(mob/living/new_owner, ...)
	. = ..()
	COOLDOWN_START(src, mammon_increase, rand(15 MINUTES, 25 MINUTES))
	required_mammons = ceil(rand(HORDE_KEEPER_START_LOW, HORDE_KEEPER_START_HIGH))

/datum/status_effect/horde_keeper/tick(wait)
	if(owner.resting) // Resting
		...
		var/has_status = !owner.has_status_effect(/datum/status_effect/horde_keeper_buff)
		if(totalvalue > required_mammons)
			...
			owner.apply_status_effect(/datum/status_effect/horde_keeper_buff)
		else
			if(!has_status)
				to_chat(owner, span_warning("My wealth... WHERE DID IT GO?!!"))
			owner.remove_status_effect(/datum/status_effect/horde_keeper_buff)
	else
		owner.remove_status_effect(/datum/status_effect/horde_keeper_buff)

/datum/status_effect/horde_keeper_buff
	id = "horde_keeper_buff"
	duration = -1 // Infinite
	alert_type = /atom/movable/screen/alert/status_effect/buff/healing/horde_keeper
	effectedstats = list(
		STATKEY_LCK = 4,
	)
	var/healing_on_tick = 3

/datum/status_effect/horde_keeper_buff/on_apply()
	. = ..()
	var/filter = owner.get_filter("horde_keeper_filter")
	if(!filter)
		owner.add_filter("horde_keeper_filter", 2, list("type" = "outline", "color" = "#c42424", "alpha" = 60, "size" = 1))
...
/datum/status_effect/horde_keeper_buff/tick()
...
/atom/movable/screen/alert/status_effect/buff/healing/horde_keeper
	name = "Horde Warmth"
