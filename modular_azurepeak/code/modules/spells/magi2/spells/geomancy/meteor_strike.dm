// Meteor Strike — Geomancy Mastery (T4 additive variant). Port of Azure-Peak meteor_strike.dm.
// A telegraphed barrage of boulders over a 7x7 zone: direct hits, splash, and gravel
// fragments. Uses our ported gravel_blast_magi2 projectile for the fragments.
...
/datum/action/cooldown/spell/meteor_strike_magi2
	button_icon = 'icons/mob/actions/mage_geomancy.dmi'
	name = "Meteor Strike"
	desc = "Call down a devastating barrage of boulders onto a wide area. \
...
/datum/action/cooldown/spell/meteor_strike_magi2/cast(atom/cast_on)
	. = ..()
...
/datum/action/cooldown/spell/meteor_strike_magi2/proc/drop_boulder(turf/T)
	if(QDELETED(src) || QDELETED(owner))
		return
	// Telegraph + falling boulder visual
	new /obj/effect/temp_visual/falling_boulder(T, CALLBACK(src, PROC_REF(meteor_impact), T))

/datum/action/cooldown/spell/meteor_strike_magi2/proc/meteor_impact(turf/T)
	if(QDELETED(src) || QDELETED(owner))
		return
...
/datum/action/cooldown/spell/meteor_strike_magi2/proc/spawn_fragments(turf/T)
