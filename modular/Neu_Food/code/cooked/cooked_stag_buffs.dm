// Buff alert screen, extracted from Azure-Peak's white_stag creature file (white_stag.dm) so the
// stag-venison eat-buff works without the full creature port (deferred). Both forest_rush (here) and
// the creature's white_rush reuse this alert; when the white_stag creature is ported later, move this
// definition there and remove it here to avoid a duplicate-definition collision.
